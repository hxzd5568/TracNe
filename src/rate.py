...
def ge_rate():
    ...
    while length_rate<=3:
        ...
        dtype = 'float32' if random.random() < 0.5 else 'float16'
        tolerance = 1e-8  if dtype == 'float16' else  1e-9
        if dtype=='float16':
            graph = gen.imagegenerate(imageshape=Input_size, imagedtype=DataType.f(16),Max_opr_num=int(Max_opr_num*length_rate)) # 1st:5+expand 10 =15; 2:
        else:
            graph = gen.imagegenerate(imageshape=Input_size, imagedtype=DataType.f(32),Max_opr_num=int(Max_opr_num*length_rate))
        ...
        # case_path = os.path.join(path,str(length_rate))
        ...
        tnum +=1
        if tnum % 10 == 0:
                    print('rate',float(vnum)/tnum)
                    print('rate16',float(vnum16)/tnum)
                    print('rate32',float(vnum32)/tnum)
        try:
            checkor = Calculate_error(mod = mod)
            if(checkor.random_difference_test()>0):
