        ...
        def fix_sensi(outputindex):
            for k,i in enumerate(outputindex):
                ...
                for temp_idx in temp_fix_node:
                    ...
                    code = re.search(r'\{.*\}', str(self.obj_func), flags= re.S|re.M).group(0)
                    sslen0 = len(re.findall('\n', code))
                    if sslen0 == 3 and 'layout_transform' in ss0:
                        continue
                    if sslen0 > 2 or sslen0 == 2 and ('softmax' in ss0 or 'tan' in ss0 or 'exp' in ss0 or 'erf' in ss0):
                        self.mod_fix()
                        print('self.newf_lastnum, self.base_num, self.onum',self.newf_lastnum, self.base_num, self.onum)
                        mod = rewrelay(self.hash, remove_virtarget(self.prim_mod),\
                                str(self.obj_func).replace('main','fn'), self.newf_lastnum, self.base_num, self.onum)
                        self.prim_mod = mod
