...
class Fuzzer(Checkor):
    ...
    def fuzzps(self, mod:IRModule=None,):
        ...
        #         self.params  = dict(f.items())
        # else:
        _, inparams = self.rundiff_changew(main_fn,)
        self.params = inparams
        for i in range(10):
            newdiff, inparams = self.rundiff_changew(main_fn,)
        ...
        for tempkey, Input_size in Input_sizes:  # for each params
            ...
            def fuzzfn(arr:np.array):# input:list of np factorymod1: tvm.runtime.Module,factorymod5: tvm.runtime.Module,
                ...
                if not os.path.exists(os.path.join(self.case_path, 'params5.npz')):
                    inputarr[self.tempkey] = np.reshape(arr, self.Input_size).astype(self.Dtype)
                    self.eparams = inputarr
                    outs1 = self.run_gmod(self.gmod1,inputarr)
                    outs5 = self.run_gmod(self.gmod5,inputarr)
                else:
            ...
            if(self.fuzzmode == 'CD'):
                ...
                ret = dual_annealing(fuzzfn, bounds=list(zip(lw, up)),maxiter=1,callback=print_fun)
                ...
                for i in range(10):
                    if random.random()<0.5:
                        x0 = np.random.uniform(size=Input_size).astype(self.Dtype).flatten()
                    else :
                        x0 = np.clip(
                                np.random.normal(size=Input_size).astype(self.Dtype).flatten()
                ...
                loaded_params[self.tempkey] = np.reshape(maxinput, self.Input_size).astype(self.Dtype)
                ...
                ret = differential_evolution(fuzzfn, bounds=bounds,
                                seed=1, maxiter= 80,init='random')
                # print("global minimum: x , f(x) = " ,ret.x,',',ret.fun,'\n')
                savearr(ret.x,self.fuzzmode)
                best = -ret.fun
                # mcmc
                ret = basinhopping(fuzzfn, ret.x ,stepsize = 0.02,
                                 niter=80,callback=print_fun)
                if -ret.fun > best:
                    savearr(ret.x,self.fuzzmode)
                ...
                for i in range(speed1):
                    ...
                    for retx, retf in simpleDE(fuzzfn, x0=x0,bounds=bounds,its=speed2,normalflag=self.dnn,dtype=self.Dtype):#15
