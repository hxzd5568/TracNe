...
Disabled_pass5 = ['FastMath',]#[ 'AlterOpLayout', 'CanonicalizeCast']
...
def MRE(y_true, y_pred,):  #precision along with  tf.keras.metrics.MeanRelativeError
    ...
    relative_error = np.average( d \
                    / (np.abs(y_true).astype(np.float64) + 1e-9) * np.abs(y_true) / np.mean(np.abs(y_true)))
    # print(y_true,y_pred,relative_error)
    return relative_error

def RE(y_true, y_pred,):  #precision along with  tf.keras.metrics.MeanRelativeError
        d = np.abs(y_true.astype(np.float64) - y_pred)
...
def build_workload(mod, params=None, Disabled_pass=['SimplifyExpr']):
        ...
        with transform.PassContext(opt_level=5,disabled_pass=Disabled_pass5):#disabled_pass=Disabled_pass
...
class Pliner:
    ...
    def pliner(self, i, j):
        ...
        exit(0)
        if (error > 1e-9 ):
            self.logging+=f'error between {i} and {m}. The error after {m} is {error}\n'
            self.pliner(i,m)
        else:
            self.logging+=f'error between {m} and {j}. The error after {m} is {error}\n'
            self.pliner(m,j)
