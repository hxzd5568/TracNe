...
speed1, speed2, speed3, speed4 = (
    ...
    1,
    5,
)  ## for non quant model 6,24,1,40  is good configuration, for quant model, 2,2 1,16 is good
topk = 100
...
class Fuzzer(Checkor):
    ...
    def fastv(self):
        ...
        for tempkey, Input_size in Input_sizes.items():  # for each params
            ...
            for i in range(3):
                ...
                if "transformer" in self.case_path:
                    ...
                    if self.oDtype == "int64":
                        ...
                        inputarr[keys[1]] = np.zeros(shape=t_sizes[1]).astype(
                            self.oDtype
                        )
            ...
            for i in range(3):
                arr = np.clip(
                    np.random.normal(loc=0, scale=0.2, size=self.Input_size), 0, 1
                )
                inputarr = dict()
                if "transformer" in self.case_path:
                    a, b = np.vsplit(np.reshape(arr, self.Input_size), [langbatch])
                    ...
                    # inputarr[self.tempkey] = np.reshape(arr, self.Input_size)
                    inputarr[self.tempkey] = np.random.randint(
                        0, 255, size=self.Input_size, dtype=self.Dtype
                    )
    ...
    def profile_nlp(self):
        ...
        if True:
            ...
            for i in range(1):
                ...
                inda = np.array(
                    sorted(outs5[0][0].tolist(), reverse=1)[0:topk]
                )  # [0][0][0][4:84] for yolov8, [0][0] for resnet [0][0][0] for
                print(outs5[0])
    ...
    def replay_error(self, report: Queue = None):
        ...
        if os.path.exists(self.case_path + "/compiled_lib0.tar"):
            self.factorymod0 = tvm.runtime.load_module(
                self.case_path + "/compiled_lib0.tar"
            )
        ...
        if "transformer" in self.case_path:
            ...
            print(
                f"top{topk} incorrect rate", (totalpredicts - correct) / totalpredicts
            )
        import onnx

        # !!! renew path
        onnx_model = onnx.load(
            "/root/.cache/sparsezoo/e37ce71a-d72f-42ec-b87f-21994e8fb6df/deployment/model.onnx"
        )
    ...
    def replay_error_int(self, report: Queue = None):
        ...
        onnx_model = onnx.load(
            "/root/.cache/sparsezoo/9273788e-f3a8-41f8-8e93-e3249621866c/deployment/model.onnx"
        )
        ...
        # print(self.MSE(outs5[1][0][inda],outs1[1][0][indb]))
        print(
            f"total top{topk} incorrect rate", (totalpredicts - correct) / totalpredicts
        )
    ...
    def profile_error(self, report: Queue = None):
        ...
        if "transformer" in self.case_path:
            ...
            indb[1:] = indb[:-1]
            print(
                "the distance between classes is", np.mean(np.abs((indb - inda) / inda))
            )
            # for i in range(nb):
            #     for j in range(nsen):
            #         inda = np.argsort(outs5[i][j])[::-1][0:5]
                ...
                if len(result0) != 0:
                    ...
                    if len(result5) == 0:
                        ...
                        if result5[0][4] != result0[0][4]:
                            ...
                            correct += 1
                            locerrors += max(
                                self.generalMSE(result5[0][0], result0[0][0]),
                                self.generalMSE(result5[0][1], result0[0][1]),
                            )
                            sizeerrors += self.generalMSE(
                                result5[0][3] * result5[0][2],
                                result0[0][2] * result0[0][3],
                            )
    ...
    def fuzzps(
        ...
        for tempkey, Input_size in Input_sizes.items():  # for each params
                ...
                if "transformer" in self.case_path:
                    ...
                    if self.oDtype == "int64":
                        if "int" in str(a.dtype):
                            ...
                            inputarr[inkeys[0]] = (a * dictnums).astype("int64")
                        inputarr[inkeys[1]] = np.zeros(shape=t_sizes[1]).astype(
                            self.oDtype
                        )
            ...
            def fuzzfn(
                ...
                if self.frame and self.usetorch:
                    ...
                    if len(inputarr) == 1:
                        outs1 = run_tmod(
                            self.torchm,
                            torch.from_numpy(list(inputarr.values())[0]).float(),
                        )
                    else:
                        outs1 = run_tmod(
                            self.torchm,
                    ...
                    if len(inputarr) == 1:
                        outs1 = get_ort_out(
                            self.onnx_model, [list(inputarr.values())[0]]
                        )
                    else:
                        outs1 = get_ort_out(
                            self.onnx_model,
                            [list(inputarr.values())[0], list(inputarr.values())[1]],
                        )
                else:
            ...
            if self.fuzzmode == "CD":
                ...
                ret = dual_annealing(
                    fuzzfn, bounds=list(zip(lw, up)), maxiter=30, callback=print_fun
                )
                ...
                ret = differential_evolution(
                    fuzzfn, bounds=bounds, seed=1, maxiter=10, init="halton"
                )
                ...
                if not self.seedq.empty():
                    ...
                    x0 = np.random.uniform(0, 1, size=self.Input_size).astype(
                        self.Dtype
                    )

                # x0 = torch.from_numpy(x0).float()
                print(self.Input_size)
                lw = boundarr(self.Boundlow, inputsize=self.Input_size).astype(
                    self.Dtype
                )
                up = boundarr(self.Boundhigh, inputsize=self.Input_size).astype(
                    self.Dtype
                )
                bounds = list(zip(lw, up))
                seeds = dict()
