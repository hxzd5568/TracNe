...
class ExprPrinter(ExprVisitor[None, Any]):
    ...
    def visit_and(self, a: And, env: None):
        self._write_cls(a)
        self._write_pos_multi(
            list(map(lambda c: lambda: self.visit(c, env), a.clauses_))
        )

    def visit_or(self, o: Or, env: None):
        self._write_cls(o)
        self._write_pos_multi(
            list(map(lambda c: lambda: self.visit(c, env), o.clauses_))
        )
    ...
    def visit_tuple(self, tup: Tuple, env: None):
        self._write_cls(tup)
        self._write_pos_multi(
            list(map(lambda f: lambda: self.visit(f, env), tup.fields_))
        )
    ...
    def visit_concat(self, concat: Concat, env: None):
        ...
        self._write_pos_multi(
            list(map(lambda arr: lambda: self.visit(arr, env), concat.arrays_))
        )
