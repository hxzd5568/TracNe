...
def visualize(
    graph: Graph, name: str, directory: str, fontname: str = "Linux Biolinum O"
):
...
class GraphVisualizer(GraphVisitor[None]):
    ...
    def visit_operation(self, opr: Operation):
        ...
        input_label = (
            "{"
            + "|".join(
                f"<i{i}>{self._fmt_ty(v.type_)}" for i, v in enumerate(opr.inputs_)
            )
            + "}"
        )
        output_label = (
            "{"
            + "|".join(
                f"<o{i}>{self._fmt_ty(v.type_)}" for i, v in enumerate(opr.outputs_)
            )
            + "}"
        )
        opr_label = (
            "{" + f"{input_label}|{opr.op_.name_}\\n"
