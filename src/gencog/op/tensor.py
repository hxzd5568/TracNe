    ...
    return TypeSpec(
        ...
        out_shapes=[
            Cond(
                ...
                Map(
                    Filter(indices, lambda i: Not(is_reduce_axis(i))),
                    lambda i: IN[0].shape[i],
                ),
            )
        ],
    )
...
def _create_expand_dims():
    return TypeSpec(
        attrs=[
            Attr(
                "axis", Var(INT, ran=Range(1 if TypeSpec.for_graph else 0, IN[0].rank))
            ),
    ...
    return TypeSpec(
        ...
        extra=[
            Subset(
                a("axis"),
                Filter(
                    indices[2:] if TypeSpec.for_graph else indices,
        ...
        out_shapes=[
            Map(
                Filter(indices, lambda i: Not(InSet(i, a("axis")))),
                lambda i: IN[0].shape[i],
            )
        ],
    )
    ...
    return TypeSpec(
        attrs=[
            Attr(
                "newshape",
                List(
                    Var(ran=dl_rank_ran if TypeSpec.for_graph else rank_ran),
        ...
        extra=[
            ...
            ReduceArray(a("newshape"), ArithOp.MUL, 1)
            == ReduceArray(IN[0].shape, ArithOp.MUL, 1),
        ],
        out_num=1,
...
def _create_transpose():
    ...
    return TypeSpec(
        ...
        in_shapes=[List(IN[0].rank, lambda _: Var(tmpl=True))],
        extra=[
            And(a("axes")[0] == 0, Perm(a("axes")[1:], indices[1:]))
            if TypeSpec.for_graph
            else Perm(a("axes"), indices)
        ],
        out_num=1,
        out_ranks=[IN[0].rank],
        out_dtypes=[IN[0].dtype],
        out_shapes=[Map(a("axes"), lambda i: IN[0].shape[i])],
    )
...
def _create_concat():
    return TypeSpec(
        attrs=[
            Attr(
                "axis",
                1 if TypeSpec.for_graph else Var(ty=INT, ran=Range(0, IN[0].rank)),
            )
        ],
        in_num=Var(ran=in_num_ran),
        in_ranks=List(
            IN.num, lambda _: Var(ran=dl_rank_ran if TypeSpec.for_graph else rank_ran)
        ),
        ...
        out_shapes=[
            List(
                ...
                lambda j: Cond(
                    j == a("axis"),
                    ReduceRange(
                        Range(end=IN.num), ArithOp.ADD, 0, lambda i: IN[i].shape[j]
                    ),
                    IN[0].shape[j],
                ),
            )
    ...
    return TypeSpec(
        attrs=[
            ...
            Attr(
                ...
                List(
                    Var(
                        ran=Range(begin=0, end=IN[0].shape[a("axis")].min(max_out_num))
                    ),
    ...
    return TypeSpec(
        attrs=[
            Attr(
                ...
                indices[2:]
                if TypeSpec.for_graph
                else List(Var(), lambda _: Var(INT, tmpl=True)),
            ),
            Attr(
                "begin",
                List(
                    num_axes,
                    lambda i: Var(INT, ran=Range(end=IN[0].shape[axes[i]]), tmpl=True),
                ),
            ),
            Attr(
                "end",
                List(
                    num_axes,
                    lambda i: Var(INT, ran=iran(1, IN[0].shape[axes[i]]), tmpl=True),
                ),
            ),
            Attr(
                "strides", List(num_axes, lambda _: Var(INT, ran=stride_ran, tmpl=True))
            ),
        ],
        in_num=1,
        in_ranks=[Var(ran=dl_rank_ran if TypeSpec.for_graph else rank_ran)],
        ...
        out_shapes=[
            List(
                ...
                lambda i: Cond(
                    InSet(i, a("axes")),
                    Map(
                        Filter(List(num_axes, lambda j: j), lambda j: axes[j] == i),
                        lambda j: (end[j] - begin[j] + strides[j] - 1) / strides[j],
                    )[0],
                    IN[0].shape[i],
                ),
            )
