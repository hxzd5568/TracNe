...
class Calculate_error:
    def __init__(
        ...
        self.case_path = path_case
        if isinstance(path_case, str):
            with open(f"{self.case_path}/code.txt", "r") as f:
                self.mod = relay.parse(f.read())
        else:
            self.mod = path_case  # mod
        self.Dtype = re.search("main.+?Tensor\[.+?\),\ (.+?)\]", str(self.mod)).group(1)
        self.eparams = None
        self.factorymod1, self.factorymod5 = build_workload(self.mod, params=params)
    ...
    def build_mod(
        ...
        if opt_level < 5:
            with transform.PassContext(
                opt_level=opt_level, disabled_pass=[self.Disabled_pass]
            ):
    ...
    def test_consistent(self):
        mod = self.mod
        try:
            rmod1 = self.build_mod(mod, 0)
        except:
            rmod1 = self.build_mod(mod, 1)
        rmod5 = self.build_mod(mod, 10)
        try:
            grmod1 = self.build_mod2(mod, 0)
        except:
            grmod1 = self.build_mod2(mod, 1)
        grmod5 = self.build_mod2(mod, 10)

        main_fn = mod["main"]
        for i in range(20):
            inputarr = self.generate_inputs(main_fn)
            outs1 = run_gmod(GraphModule(rmod1["default"](dev)), inputarr)
            outs5 = run_gmod(GraphModule(rmod5["default"](dev)), inputarr)

            outs12 = run_gmod(grmod1, inputarr)
            outs52 = run_gmod(grmod5, inputarr)
            tempdiff = 0
            print("175")
            for i, (ro, o) in enumerate(zip(outs1, outs12)):
                diff = self.MSE(ro, o)
                tempdiff = max(diff, tempdiff)
            outdiff = max(diff, tempdiff)
            for i, (ro, o) in enumerate(zip(outs5, outs52)):
                diff = self.MSE(ro, o)
                tempdiff = max(diff, tempdiff)
            outdiff = max(diff, tempdiff)
            print(outdiff)
