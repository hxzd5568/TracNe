...
Disabled_pass5 = [
    "SimplifyExpr"
]  # ['AlterOpLayout','ForwardFoldScaleAxis']#[ 'AlterOpLayout', 'CanonicalizeCast']#['AlterOpLayout','ForwardFoldScaleAxis']#[ 'AlterOpLayout', 'CanonicalizeCast']#['CanonicalizeOps','BackwardFoldScaleAxis', 'FoldConstant', 'FastMath', 'ForwardFoldScaleAxis', 'SimplifyExpr']
sys.path.append("../")
...
def build_workload(
    path, path2, params=None, Disabled_pass=[], OPTLEVEL=5, fuseopsmax=64
):  #'SimplifyExpr'
    print("come here")
...
class Checkor:
    def __init__(
        ...
        self.eparams = None
        if os.path.exists(self.case_path + "/compiled_lib1.tar") and os.path.exists(
            self.case_path + "/compiled_lib5.tar"
        ):
            self.factorymod1 = tvm.runtime.load_module(
                self.case_path + "/compiled_lib1.tar"
            )
            self.factorymod5 = tvm.runtime.load_module(
                self.case_path + "/compiled_lib5.tar"
            )
        else:
            self.factorymod1, self.factorymod5 = build_workload(
                f"{self.case_path}/code.txt",
                self.case_path,
                params=params,
        ...
        self.tolerance = (
            (1e-3 if self.Dtype == "float16" else 1e-6) if self.dnn is None else 1e-7
    ...
    ## basic operations
    def run_gmod(
        self, gmod: GraphModule, inputs: Dict[str, np.ndarray] = None
    ) -> List[np.ndarray]:
    ...
    def locate_naninf(self, modstr: str):
        ...
        if modstr == "1":
            ...
            graph_mod = GraphModuleDebug(
                self.factorymod1["debug_create"]("default", dev),
                [dev],
                self.factorymod1["get_graph_json"](),
                dump_root=dump_root,
            )
        else:
            print(type(self.factorymod5))
            graph_mod = GraphModuleDebug(
                self.factorymod5["debug_create"]("default", dev),
                [dev],
                self.factorymod5["get_graph_json"](),
                dump_root=dump_root,
            )
    ...
    def replay_withsample(self):
        ...
        for i in range(int(2e4)):
            delta = np.random.uniform(
                0, 1e-3, size=list(loaded_params.values())[0].shape
            )
        ...
        for i in range(int(2e4)):
            delta = np.random.uniform(
                0, 1e-4, size=list(loaded_params.values())[0].shape
            )
            loaded_params[key] = delta + tmp
            outs1 = self.run_gmod(self.gmod1, loaded_params)
            outs5 = self.run_gmod(self.gmod5, loaded_params)
    ...
    def replay_withdebugger(self, report: Queue = None):
        ...
        path_params = os.path.join(self.case_path, "inputs.npz")
        graph_mod1 = GraphModuleDebug(
            self.factorymod1["debug_create"]("default", dev),
            [dev],
            self.factorymod1["get_graph_json"](),
            dump_root=self.case_path + "/L1/",
        )
        graph_mod5 = GraphModuleDebug(
            self.factorymod5["debug_create"]("default", dev),
            [dev],
            self.factorymod5["get_graph_json"](),
    ...
    def replay_withnewgmod(self, mod: IRModule = None):
        ...
        try:
            with transform.PassContext(
                opt_level=opt_level + 1, disabled_pass=self.Disabled_pass
            ):
                mod1 = relay.build(
                    mod,
    ...
    def build_mod(
        self, mod: IRModule, opt_level: int, params: Optional[TensorDict] = None
    ):
        if opt_level == 0:
            with transform.PassContext(
            ...
            with transform.PassContext(
                opt_level=opt_level, required_pass=self.Required_pass
            ):
                lib = relay.build(mod, target, params=params)
        return lib

    def generate_inputs_nameshape(
        self, main_fn: relay.function.Function, inputs_num: int = 1
    ):
        length = inputs_num
        inputs_shape = dict()
    ...
    def generate_inputs(
        ...
        if arrs is None:  # random test
            ...
            for i in range(length):
                ...
                var_tyx = varx.checked_type
                size = [int(d) for d in var_tyx.shape]
                inputarr[varx.name_hint] = np.random.normal(0, 0.1, size=size).astype(
                    var_tyx.dtype
                )
            ...
            for j in range(speclen):
                ...
                inputarr[varx.name_hint] = np.clip(
                    arrs[j].astype(var_tyx.dtype), self.Boundlow, self.Boundhigh
                )  # one 2 many arrs[i]
        return inputarr
