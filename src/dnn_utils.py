...
class Checkor:
    ...
    def MSE(
        ...
        relative_error = np.average(
            np.abs(y_true - y_pred).astype(np.float64)
    ...
    def MRE(
        ...
        y_true,
        y_pred,
    ):  # signal exposing numerical bugs
        flag = 0
    ...
    def replay(self):
        ...
        try:
            with transform.PassContext(
                opt_level=opt_level + self.bigflag, disabled_pass=self.Disabled_pass
            ):
                mod1 = relay.build(
                    mod,
