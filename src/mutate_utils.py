    ...
    def f3(self, indata: np, order: str):  # add uniform noise
        if indata.ndim == 2 and indata.shape == (128, 3072):
            ...
            delta = np.random.uniform(
                low=inmean / 100, high=inmean / 10, size=ins.shape
            )
            rseed = np.random.random(1)
            if rseed <= 0.2:
...
def simulated_annealing(objective, bounds, its=150, x0=None, step_size=0.1, temp=20):
    ...
    for i in range(its):
        ...
        candidate = np.clip(
            candidate,
            np.reshape(bounds[:, 0], xshape),
            np.reshape(bounds[:, 1], xshape),
        )
...
def encode(bounds, n_bits, value):
    ...
    for i in range(len(bounds)):
        integer = int(
            (value[i] - bounds[i][0]) / (bounds[i][1] - bounds[i][0]) * largest
        )
...
def pmutation(bitstring, r_mut):
    ...
    bitstring = (
        (1 - bitmat) * np.equal(changeloc, 1) + bitmat * np.equal(changeloc, 0)
    ).tolist()
...
def genetic_algorithm(
    ...
    best, best_eval = x0[0], objective(
        x0[0].astype(type)
    )  # decode(bounds, n_bits, pop[0])
    children = list()
