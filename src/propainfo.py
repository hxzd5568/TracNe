...
def rewrelay(hash:str,ocode,rcode,newf_lastnum,base_num, onum):
    ...
    #print(ocode[r:])
    poststr = re.sub('(?P<value>%\d+)', update_external, ocode[r:],count=0, flags=re.M|re.S)
    #print(poststr)
    prestr = re.sub('(?P<value>%\d+)',  update_external, ocode[:l],count=0, flags=re.M|re.S)
    new_func = re.sub('(?P<value>%\d+)',update_internal, new_func, count=0, flags=re.M|re.S )
...
class Corrector:
    def __init__(self, dumppath) -> None:
        ...
        with open(modpath,'r') as fp:
            ...
            self.pbatch = int(int(re.findall('(%\d+)',str(self.mod))[-1].strip('%'))/10+1)
        ...
        self.debpath1= os.path.join(dumppath,'L1/_tvmdbg_device_CPU_0/_tvmdbg_graph_dump.json')
        self.debpath5= os.path.join(dumppath,'L5/_tvmdbg_device_CPU_0/_tvmdbg_graph_dump.json')
    ...
    def verify_mixed_precision_(self,
    ...
    def hack_build_graph(self, target='llvm'):
        def get_mod_hash_callfunction():
                ...
                return [self.irmodule.__getitem__(funcnames[i]).attrs['hash'] for i in range(lens)]
        def get_function_fromhash(hash,ocode):
            ...
            a = matched1.group()
            matched  =  re.search(pattern2, ocode[:matched1.span()[1]][::-1], flags=re.M|re.S)
            b = matched.group(1)[::-1]
            origin_func = b+a
        ...
        # get base number
        matchedall = re.findall(r'(%\d+ = )', funcstr[funcstr.find('{'):])  # change handle base mnumber
        if len(matchedall) >= 1:
    ...
    def prim_valid_fix(self):
        ...
        if os.path.exists(self.dumpath+"/compiled_lib1.tar"):
            self.factorymod1 = tvm.runtime.load_module(self.dumpath+"/compiled_lib1.tar")
        ...
        if error_after_fix > self.tolerance:
            if os.path.exists(self.dumpath+'-h/code.txt'):
                ...
                error_after_fix2 = calculator.replay_withlocatenan_withtar1(inp_path, hgmod1)
                if error_after_fix2 < self.tolerance:
                    print('opt are more precious',error_after_fix2)
                else:
                    factorymod5 = tvm.runtime.load_module(self.dumpath+"/compiled_lib5.tar")
                    tar5 = GraphModule(factorymod5["default"](dev))
                    error_after_fix3 = calculator.replay_withlocatenan_withtar5(inp_path, hgmod1,tar5)
            ...
            with open(fixportpath,'a') as fp:
                ...
                fp.write('\n\n\n'+'*'*50+'\n')
                fp.write('After repairing, error prone input\'s mean relatively error is: %.10f'\
                          %(error_after_fix)) #
                fp.write('\n\n\n'+'*'*50+'\n')
    ...
    def mod_fix_and_check(self,input):
        ...
        self.obj_func = self.verify_mixed_precision_(self.obj_func, mod_params=input,eouts=self.eouts)
...
#         exit()
        ...
        # 1 get output node
        tlength = self.nodedict5['node_row_ptr'][-1]-1
        def get_out_node():
            ...
            return outputindex
        def get_out_node_from_debug():
            outputindex = []
            outs = self.nodedict5['heads']
            for i in outs:
                out = i[0]
                outputindex.append(out)
            return outputindex
        outputindex = get_out_node_from_debug()
        print('outputindex', outputindex)
        # print
        with open(self.dumpath+'/error_accumulation.txt','a') as fp:
            print('==='*3,file=fp)
            print(str((self.dumpath).split('/')[-1]),file=fp)
            for k,i in enumerate(outputindex):
                ...
                if self.trace_error_dict[str(i)]['errormessage']['error'] >0 :
                    ...
                    # currentindex = i-1
                    # if currentindex>0 and self.trace_error_dict[str(currentindex)]['errormessage']['error'] >0 :
                    #         prenode_index = self.trace_error_dict[str(currentindex)]['pre_topoindex']
                    #         sensi_nodes = [currentindex]
                    #         break
                    # currentindex = i+1
                    # print(currentindex, tlength)
                    # if currentindex<=tlength and self.trace_error_dict[str(currentindex)]['errormessage']['error'] >0 :
                    #         prenode_index = self.trace_error_dict[str(currentindex)]['pre_topoindex']
                    #         sensi_nodes = [currentindex]
                    #         break
                    continue
                    while(currentindex!=0):
                        if self.trace_error_dict[str(currentindex)]['errormessage']['error'] >0 :
                            prenode_index = self.trace_error_dict[str(currentindex)]['pre_topoindex']
                            sensi_nodes = [currentindex]
                            break
                        currentindex-=1
                ...
                print('-chain-   '+str(k)+' th',file= fp)
                # trace back along each chain
                # handle current chain
                if -1 in prenode_index:
                    prenode_index.remove(-1)
                zero_nodes = [ i for i in prenode_index if self.trace_error_dict[str(i)]['errormessage']['error'] ==0 ]
                ...
                while(len(prenode_index)!=0 ):  # bfs
                    ...
                    for nidx in newidx:
                        if nidx!= -1 and\
                            self.trace_error_dict[str(nidx)]['errormessage']['error'] != 0 \
                            :
                ...
                for i in sensi_nodes:
                    keys = self.trace_error_dict[str(i)]['errormessage']['node_name_hint']
                    vals = self.trace_error_dict[str(i)]['errormessage']['error']
                    if 'tvm' in keys:
                        print(str(i)+'  '+keys+','+str(vals),file=fp)
                print('',file=fp)
