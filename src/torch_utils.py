...
def build_tvm(baseline_model, baseline_input):
    trace = torch.jit.trace(
        baseline_model, [input.clone() for input in baseline_input], strict=False
    )
    if isinstance(baseline_model, torch.nn.Module):
...
def test_dnn(
    ...
    if "transformer" in case_path or "rnn" in case_path or "vgg" in case_path:
        ...
        low, high = pretrainedmodels.pretrained_settings[model_name.rstrip("f16")][
            "imagenet"
        ]["input_range"]
        input_size = pretrainedmodels.pretrained_settings[model_name.rstrip("f16")][
            "imagenet"
        ]["input_size"]
    ...
    if not os.path.exists(case_path + "/model_scripted.pt") and "f16" not in case_path:
        if "transformer0" in case_path:
            ...
            tokenizer = AutoTokenizer.from_pretrained(
                "/home/user/tvm/models/transformerprune_d2"
            )
            model = AutoModel.from_pretrained(
                "/home/user/tvm/models/transformerprune_d2", from_tf=True
            )
            ...
            dummy_input2 = torch.from_numpy(
                np.random.randint(0, 1e4, size=(15, 32, 768))
            )
