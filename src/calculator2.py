...
def build_workload(mod, params=None, Disabled_pass=["SimplifyExpr"], isolatepass=[]):
...
class Calculate_error:
    ...
    def build_mod(self, mod: IRModule, opt_level: int, isolatepass=[], params=None):
        if opt_level <= 1:
        ...
        if arrs is None:  # random test
            ...
            for i in range(length):
                ...
                inputarr[varx.name_hint] = np.clip(
                    np.random.normal(0, 1, size=size).astype(var_tyx.dtype),
                    self.Boundlow,
                    self.Boundhigh,
                )
