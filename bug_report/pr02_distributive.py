...
def MSE(
    ...
    relative_error = np.average(d / (np.abs(y_true).astype(np.float64) + 1e-8))
...
def replay(mod, params):
    factorymod1, factorymod5 = build_workload(mod, params=params)
...
def test_mod1():
    ...
    params = {
        "x": np.array(
            [
                [-3.0407448, 5.0, 1.4677091],
                [5.0, -0.08194685, 3.0596933],
                [5.0, 5.0, 3.7800522],
                [5.0, 3.1617928, 5.0],
            ],
            dtype=np.float32,
...
def test_mod3():
    def mod3():
        ...
        f = relay.const(3.0)
        m = relay.nn.conv2d(
            data, weight, padding=[1, 1, 1, 1], channels=3, kernel_size=[3, 3]
        )
    ...
    params = {
        ...
        "weight": np.array(
            [
                [
                    [
                        [-0.036727745, -0.08940563, 0.05051272],
                        [-0.0333286, 0.21914756, -0.12153826],
                        [-0.07797686, 0.009578303, 0.073190555],
                    ],
                    [
                        [-0.2484066, 0.000455132, -0.21513228],
                        [-0.018433733, -0.2710633, -0.08242117],
                        [-0.13191707, -0.098520845, 0.22458874],
                    ],
                    [
                        [0.035224877, 0.16323657, -0.09220455],
                        [0.22315206, -0.072422385, -0.12208488],
                        [0.19187246, -0.07688672, 0.003581573],
                    ],
                ],
                [
                    [
                        [0.027952887, 0.061361663, -0.024426105],
                        [0.07162253, -0.14932604, 0.06435288],
                        [-0.008233514, 0.08305748, -0.17171563],
                    ],
                    [
                        [-0.09118227, -0.014926386, 0.014232294],
                        [-0.1564838, -0.069641225, 0.061708245],
                        [0.072204806, 0.11291813, 0.11106545],
                    ],
                    [
                        [-0.24633893, 0.057572417, 0.08551504],
                        [-0.12791695, -0.011270673, 0.13212293],
                        [-0.1458071, 0.1949932, 0.007811096],
                    ],
                ],
                [
                    [
                        [-0.10390888, -0.049856093, -0.08322507],
                        [-0.046482403, -0.17208004, 0.08034821],
                        [0.0440737, -0.003398159, 0.09506506],
                    ],
                    [
                        [-0.08517007, 0.010996343, 0.12853315],
                        [-0.023498693, 0.071802065, -0.042154644],
                        [-0.004092929, 0.12372954, 0.0930078],
                    ],
                    [
                        [-0.055090867, -0.07654511, -0.03721075],
                        [0.030530095, -0.07391206, 0.036769908],
                        [-0.13674302, -0.056045175, -0.010012659],
                    ],
                ],
            ],
            dtype=np.float32,
        ...
        "in_bias": np.array(
            [[[-0.10464073]], [[0.0493139]], [[-0.10237443]]], dtype=np.float32
        ),
    }
    replay(mod, params)
