    ...
    for i in range(length):
        ...
        if random.random() < 0.3:
            inputarr[varx.name_hint] = np.clip(
                np.random.normal(size=size).astype(var_tyx.dtype), Boundlow, Boundhigh
            )
        elif random.random() < 0.6:
            inputarr[varx.name_hint] = np.clip(
                np.random.uniform(size=size).astype(var_tyx.dtype), Boundlow, Boundhigh
            )
        elif random.random() < 0.9:
            inputarr[varx.name_hint] = np.clip(
                np.random.standard_cauchy(size=size).astype(var_tyx.dtype),
                Boundlow,
                Boundhigh,
            )
        else:
